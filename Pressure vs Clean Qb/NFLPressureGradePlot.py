...
with open(pressure_data, 'r', newline='') as infile:
    ...
    for row in reader:
        name = row[0]
        pressure_grade_str = row[5]
        clean_grade_str = row[6]
        num_dropbacks = int(row[7])

        # Add validation for minimum number of dropbacks
        if pressure_grade_str.strip() and clean_grade_str.strip() and num_dropbacks >= 5:
...
plt.annotate('Great Pressured but not Clean?', (30,90), fontsize=7, color = 'purple')
plt.annotate('Poor Clean and Pressured', (30,30), fontsize=7, color = 'red')
plt.annotate('Poor Pressured but Great Clean', (90,30), fontsize=7, color = 'purple')

# Add a title
plt.suptitle(f' 2022 NFL PFF Clean Pocket Grade vs. PFF Under Pressure Grade', fontsize=15)
plt.title(f'Min. 5 Dropbacks \nLeague Averages: Clean Pocket Avg. ({clean_avg:.2f}), Pressured Avg. ({pressure_avg:.2f})', fontsize=7, loc = 'left')
